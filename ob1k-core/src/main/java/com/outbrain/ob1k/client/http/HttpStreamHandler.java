...
public class HttpStreamHandler<T> implements AsyncHandler<T> {
    ...
    public STATE onBodyPartReceived(final HttpResponseBodyPart bodyPart) throws Exception {
        ...
        if (body.startsWith(ChunkHeader.ELEMENT_HEADER)) {
            ...
            ((NettyResponseBodyPart) bodyPart).getChannelBuffer().readerIndex(ChunkHeader.ELEMENT_HEADER.length());
            ...
            //advance the read index beyond the chunk header.
            ((NettyResponseBodyPart) bodyPart).getChannelBuffer().readerIndex(ChunkHeader.ERROR_HEADER.length());
